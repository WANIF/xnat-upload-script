...
# Batch size: number of scans to upload before waiting
batch_size = 300
# Wait time: time to wait, in seconds, between batches
wait_time = 60*5
...
with xnat.connect('https://xnat-mri.cmca.uwa.edu.au') as session:
    ...
    scan_counter = 1    # Number of scans processed so far - so we can pause in between
    # Upload and then delete each dicom file
    for f in glob.glob(f"{scandir}/**/*.dcm"):
        # If a batch is complete, wait wait_time until continuing
        if scan_counter % batch_size == 0:
            print("Batch complete: waiting to try next batch...\n")
            time.sleep(wait_time)
            # Here you can try archiving any prearchived sessions
            print("Continuing...")
        ...
        try:
            ...
            with open("upload_log.txtz", "a") as log:
                log.write(f"{f}\n")
